...
export class PersonRepository {
  ...
  getFaceForFacialRecognitionJob(id: string) {
    return this.db
      .selectFrom('asset_face')
      .select(['asset_face.id', 'asset_face.personId', 'asset_face.sourceType', 'asset_face.frameCount'])
      .select((eb) =>
        jsonObjectFrom(
          eb
            .selectFrom('asset')
  ...
  getDataForThumbnailGenerationJob(id: string) {
    return this.db
      ...
      .select([
        ...
        'asset_face.thumbnailIndex as thumbnailIndex',
        'asset.type',
        'asset.duration',
        'asset.originalPath',
  ...
  getBestFace(personId: string) {
    return this.db
      ...
      .where('asset_face.deletedAt', 'is', null)
      .where('asset_face.isVisible', 'is', true)
      .orderBy('asset_face.score', 'desc') // Select the best frame shot (highest score)
      .executeTakeFirst();
  }
